...
workers.validateCheckData = function (originalCheckData) {
  ...
  originalCheckData.successCodes =
    typeof originalCheckData.successCodes == "object" && originalCheckData.successCodes instanceof Array && originalCheckData.successCodes.length > 0 ? originalCheckData.successCodes : false;

  originalCheckData.timeoutSeconds =
    typeof originalCheckData.timeoutSeconds == "number" && originalCheckData.timeoutSeconds % 1 === 0 && originalCheckData.timeoutSeconds >= 1 && originalCheckData.timeoutSeconds <= 5
      ? originalCheckData.timeoutSeconds
      : false;
...
//Alert the user as to a change if their hceck status
workers.alertUserToStatusChange = function (newCheckData) {
  var msg = "Alert: Your check for " + newCheckData.method.toUpperCase() + " " + newCheckData.protocol + "://" + newCheckData.url + " is currently " + newCheckData.state;
  helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
    if (!err) {
      debug("Succes: User was alerted to a status change in their check, via sms: ", msg);
    } else {
...
workers.log = function (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) {
  ...
  //apppend the log string to the file
  _logs.append(logFileName, logString, function (err) {
    if (!err) {
      debug("loggin to file successed");
    } else {
        ...
        _logs.compress(logId, newFileId, function (err) {
          if (!err) {
            ...
            _logs.truncate(logId, function (err) {
              if (!err) {
                debug("Succes truncateing logFile");
              } else {
                debug("Error truncating logFfile");
              }
            });
          } else {
...
workers.init = function () {
  ...
  //Call the compression loop so logs will be compression later on
