...
server.unifiedServer = function (req, res) {
  ...
  req.on("end", function () {
    ...
    var chosenHandler = typeof server.router[trimePath] != "undefined" ? server.router[trimePath] : handlers.notFound;
    //if the request is whithin the public director,use the public handler instead
    chosenHandler = trimePath.indexOf("public/") > -1 ? handlers.public : chosenHandler;
    ...
    chosenHandler(data, function (statusCode, payload, contentType) {
      ...
      if (statusCode == 200) {
        debug("\x1b[32m%s\x1b[0m", method.toUpperCase() + " /" + trimePath + " " + statusCode);
      } else {
        debug("\x1b[31m%s\x1b[0m", method.toUpperCase() + " /" + trimePath + " " + statusCode);
      }
    });
  });
};
